window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var TOKENS = {
			...
			'--dsw-alias-bg-layer-3': '#1b1d23',
			'--dsw-alias-bg-overlay': '#21252b',
			'--dsw-alias-bg-skeleton': '#2c313c',
			'--dsw-alias-bg-multi-select': '#21252b',
			...
			'--dsw-alias-border-l1': '#3e4452',
			'--dsw-alias-border-l2': '#4b5362',
			'--dsw-alias-border-l3': '#5c6370',
			'--dsw-alias-border-l4': '#404754',
			'--dsw-alias-border-l2-darkmode-thin': '#181a1f',
			'--dsw-alias-border-inverted': '#181a1f',
			'--dsw-alias-border-inverted2': '#343a45',
			'--dsw-alias-brand-primary': '#61afef',
			'--dsw-alias-brand-primary-new-colorprimary-new-color': '#61afef',
			'--dsw-alias-brand-primary-invert': '#f8fafd',
			'--dsw-alias-brand-text': '#f8fafd',
			...
			'--dsw-alias-label-dimmed': '#6b717d',
			'--dsw-alias-label-primary-dimmed': '#7f848e',
			'--dsw-alias-label-primary-bluish': '#d7dae0',
			'--dsw-alias-label-primary-foreground': '#282c34',
			...
			'--dsw-alias-interactive-bg-active': '#2c313a',
			'--dsw-alias-interactive-bg-hover-accent': '#323842',
			'--dsw-alias-interactive-bg-hover-danger': '#e06c7526',
			'--dsw-alias-interactive-bg-hover-solid': '#3e4451',
			'--dsw-alias-markdown-code-block': '#21252b',
			'--dsw-alias-markdown-code-block-banner': '#1b1d23',
			...
			'--dsw-alias-button-info-label': '#fff',
			'--dsw-alias-button-floating-fill': '#21252b',
			'--dsw-alias-button-floating-hover': '#323842',
			'--dsw-alias-button-ghost-active-fill': '#2c313c',
			'--dsw-alias-button-ghost-active-hover': '#323842',
			'--dsw-alias-button-ghost-active-border': '#3e4452',
			'--dsw-alias-button-elevated-fill': '#30333d',
			'--dsw-alias-button-contrast-fill': '#f8fafd',
			'--dsw-alias-button-tool-bar-fill-invisible': '#1f1f1f5c',
			'--dsw-alias-button-tool-bar-fill': '#3e4452',
			'--dsw-alias-button-tool-bar-hover': '#4b5362',
			'--dsw-alias-toast-bg': '#21252b',
			'--dsw-alias-tooltip-bg': '#21252b',
			'--dsw-alias-scrollbar-bg-l1': '#4e5666',
			'--dsw-alias-scrollbar-bg-l2': '#5a6375',
			'--dsw-alias-scrollbar-hover-l1': '#747d91',
			'--dsw-alias-scrollbar-hover-l2': '#8a94a6',
			'--dsw-specific-sidebar-fill': '#21252b',
			'--dsw-specific-sidebar-nav-item-active': '#2c313c',
			'--dsw-specific-sidebar-nav-item-hover': '#2c313c',
			'--dsw-specific-sidebar-nav-item-active-accent': '#2c313c',
			...
			'--dsw-alias-accent': '#61afef',
			'--dsw-alias-accent-ink': '#fff',
			'--dsw-alias-accent-primary': '#61afef',
			'--dsw-alias-accent-soft': '#2c313c',
			'--dsw-alias-border': '#3e4452',
			'--dsw-alias-border-default': '#3e4452',
			'--dsw-alias-danger': '#e06c75',
			'--dsw-alias-fill-l2': '#2c313c',
			'--dsw-alias-fill-tsp-secondary': '#2c313c',
			'--dsw-alias-hairline': '#3e4452',
			'--dsw-alias-label-error': '#e06c75',
			'--dsw-alias-label-quaternary': '#6b717d',
			'--dsw-alias-line-secondary': '#3e4452',
			'--dsw-alias-separator-primary': '#5c6370',
			'--dsw-alias-state-error-bg': '#e06c7526',
			'--dsw-alias-state-error-border': '#e06c7566',
			'--dsw-alias-state-warn-bg': '#3a3226',
			'--dsw-alias-state-warn-border': '#e5c07b80'
		};
		...
		function apply(ctx) {
			...
			// 插件 Loader row 持久化（写到 profile patch 的 entry config），经插件自有路由读写。
			...
			ctx.effect(function () {
				return ctx.on('theme/change', function () {
					if (disposed) return;
					if (preferred !== 'one-dark-pro') return;
					// 恢复必须延后到本次 theme/change 分发结束之后：若在分发途中同步
					// setTheme，恢复事件会嵌套在回退事件内层先派发完，注册在本守卫
					// 之后的监听者（如外观四宫格行）会最后收到过期的回退事件，把
					// 界面打回上一个选择。微任务里按当前快照判断，过期事件自然失效。
					Promise.resolve().then(function () {
						if (disposed || preferred !== 'one-dark-pro') return;
						if (theme.getTheme().preference === 'one-dark-pro') return;
